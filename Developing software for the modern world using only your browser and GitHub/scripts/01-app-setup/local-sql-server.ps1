...
docker run -e "ACCEPT_EULA=Y" `
            -e "SA_PASSWORD=AStrong@Passw0rd" `
            -p 1420:1433 `
            --name sql1 `
            -h sql1 `
            -v "/var/docker/sql1/data:/var/opt/mssql/data" `
            -v "/var/docker/sql1/log:/var/opt/mssql/log" `
            -v "/var/docker/sql1/secrets:/var/opt/mssql/secrets" `
